...
На гейте `proposal` CLI показывает расхождения (`conflicts`): противоречия запросу из Evidence Pack и отступления proposal с решениями планировщика. Человек видит, где план отклонился от его слов, до утверждения, а оркестратор хоста показывает список дословно, не пересказывая сообщения субагентов.
...
deviations:                  # только planner на фазе propose: отступления proposal от запроса или evidence
...
Расхождения. Исследователь не пересказывает запрос: раздел «Разбор запроса» и поле `request` перечисляют его явные утверждения дословными цитатами со статусом. Строки со статусом `противоречит` и все `deviations` планировщика CLI сохраняет в `change.yaml` (`conflicts`, идентификаторы C1, C2… перенумеровываются при каждом отчёте) и показывает на гейте `proposal` в ответе `sbox next`. Проверки в `report`: отсутствие `request` у исследователя это ошибка `REQUEST_MAP_MISSING`; цитата, не найденная в запросе после нормализации (регистр, ё, кавычки, тире, пробелы), даёт предупреждение `REQUEST_QUOTE_MISMATCH`, а одна цитата почти на весь запрос предупреждение `REQUEST_QUOTE_BLANKET`; если исследователь отметил противоречия, а планировщик не вернул `deviations`, отчёт планировщика отклоняется с `DEVIATIONS_MISSING`. Проверка не судит о смысле: она делает пересказ запроса невозможным, а расхождение видимым человеку до утверждения. Предупреждения не запускают круг доработки.
...
Команда `report` принадлежит оркестратору хоста или раннеру, роли её не вызывают: пакет не содержит команды отчёта, а шаблон агента это запрещает. `report` без `--file` берёт `runs/<следующий>/result.md`; если его нет, а последний запуск той же роли переписал свой ответ после отклонённого отчёта, принимается он. Повторный `report` с уже принятым содержимым (тот же sha, что в receipt) не создаёт запуск и возвращает прежний результат с диагностикой `REPORT_ALREADY_APPLIED` и полем `alreadyApplied`. В ответе есть строка `summary` для человека: роль, запуск, статус, артефакты, расхождения и следующий шаг; скилл показывает её дословно.
